...
/// и за запуск загрузки сначало Addressables сцены, затем, если не удасца её загрузить то загружает обычную сцене(через SceneManager)
/// (нужен именно в таком ввиде, т.к загрузку сцен надо делать ПОЛСЕ обновлений)
/// </summary>
public class TaskLoggerUpdateAllObjectAndLoadSceneAddressablesAndLocal : AbsTaskLoggerLoaderDataMono
{
     [Header("---------")]
    [SerializeField]
    ...
    [SerializeField]
    private LoadLocalDefaultAndRemoteAddressablesScene _sceneLoad;
