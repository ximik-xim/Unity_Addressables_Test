...
[CreateAssetMenu(menuName = "Save Data/Storage Data/Storage/String/Prefs")]
public class SD_StorageDataStringPrefs : SD_AbsStringStorage,IInitScripObj
{
    public override event Action OnInit;
    public override event Action OnUpdateData;
    public override event Action<SD_KeyStorageStringVariable> OnUpdateValue;
    ...
    /// </summary>
    [SerializeField] 
    private SD_SaveDataStringDefListData _saveDataStringDefListData = new SD_SaveDataStringDefListData();
    
    /// <summary>
    /// Дефолтные значения переменных
    /// </summary>
    [SerializeField] 
    private List<SD_GetClassKeyDataGetDKOString> _defaultValueVariable = new List<SD_GetClassKeyDataGetDKOString>();
    private Dictionary<string, string> _keyDefaultValueVariable = new Dictionary<string, string>();
  ...
  InitializationStorage();
    ...
    private void InitStorage()
    {
        ...
        SD_SaveDataStringDefListData data = JsonUtility.FromJson<SD_SaveDataStringDefListData>(dataJS);
        
        if (data == null)
        {
            Debug.LogWarning("Внимание, пришел null с данными в Storage");         
            _saveDataStringDefListData = new SD_SaveDataStringDefListData();
        }
        else
        {
            _saveDataStringDefListData = data;
        }
    ...
    private  void StartSaveData()
    {
        ...
        string data = JsonUtility.ToJson(_saveDataStringDefListData);
    ...
    public override bool IsThereData(SD_KeyStorageStringVariable dataKey)
    {
        return _saveDataStringDefListData.IsThereData(dataKey.GetKey());
    }

    public override string GetData(SD_KeyStorageStringVariable dataKeyVariable)
    {
        ...
        return _saveDataStringDefListData.GetValue(dataKeyVariable.GetKey());
    }

    public override void SetData(SD_KeyStorageStringVariable dataKey, string data)
    {
        _saveDataStringDefListData.SetValue(dataKey.GetKey(), data);
    ...
    public override void AddDefValue(SD_KeyStorageStringVariable key, string data)
    {
        _keyDefaultValueVariable.Add(key.GetKey(),data);
    }

    public override string GetDefValue(SD_KeyStorageStringVariable key)
    {
        return _keyDefaultValueVariable[key.GetKey()];
    }

    public override bool IsThereDataDef(SD_KeyStorageStringVariable dataKey)
    {
        return _keyDefaultValueVariable.ContainsKey(dataKey.GetKey());
    }

    public override IReadOnlyList<SD_KeyStorageStringVariable> GetListKey()
    {
        List<SD_KeyStorageStringVariable> listKey = new List<SD_KeyStorageStringVariable>();
       
        foreach (var VARIABLE in _saveDataStringDefListData.GetListKey())
        {
            listKey.Add(new SD_KeyStorageStringVariable(VARIABLE));
        }
